...
@app.route('/send_email/<string:id>', methods=["POST"])
def send_email(id):
    pass
...
def digital_books(): 
    ...
    connection = get_db_connection()
    cur = connection.cursor()
    result = cur.execute("SELECT id,name,author,url from digitalbooks")
    books = cur.fetchall()
    return render_template("digital_books.html",  books=books)
    
# Define add-book form
class AddDigitalBook(Form):
    id = StringField('Book ID', [validators.Length(min=1, max=111)])
    name = StringField('Name', [validators.Length(min=2, max=255)])
    author = StringField('Author(s)', [validators.Length(min=2, max=255)])
    url = StringField('URL', [validators.Length(min=2, max=255)])
...
def add_digital_book():
    ...
    if request.method == 'POST' and form.validate():
        ...
        # Execute SQL Query
        cur.execute("INSERT INTO digitalbooks (id, name, author, url) VALUES (%s,%s,%s,%s)",[
            form.id.data,
            form.name.data,
            form.author.data,
            form.url.data
